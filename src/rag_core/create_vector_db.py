...
def main():
    ...
    try:
        ...
        chunks = creator.load_chunks_from_json("../../data/output_chunks.json")

        # 3. Tạo Langchain Documents
        documents = creator.create_documents(chunks)

        # 4. Tạo vector store
        creator.create_vector_store(documents)
        ...
        results = creator.search_with_scores(test_query, k=3)  # Use search_with_scores to get tuples

        print(f"\nTest query: '{test_query}'")
        print("Top 3 results:")
        for i, (doc, score) in enumerate(results):  # Unpack the tuple
